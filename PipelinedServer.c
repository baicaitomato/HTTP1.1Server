...
int main(int argc, char * argv[]){
    ...
    while(1){
        ...
        if (pid < 0){
            ...
            if (setsockopt (client_fd, SOL_SOCKET, SO_RCVTIMEO, &time,
                sizeof time) < 0){
            ...
            while (read(client_fd, client_msg, msg_size) > 0 && status == 1){
                ...
                curr = client_msg;
                while((part = strstr(curr, "\r\n\r\n")) != NULL) {
                    size = part - curr + 1;
                    part_copy = (char *)malloc(size * sizeof(char));
                    // allocate message
                    memset(part_copy, '\0', size);
                    strncpy(part_copy, curr, size);
                    status = handle_request(client_fd, part_copy, argv[2]);
                    // free space 
                    free(part_copy);
                    curr = part + 4; // go forward to skip \r\n
                }
            }
